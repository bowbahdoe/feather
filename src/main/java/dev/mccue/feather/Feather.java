...
public final class Feather {
    ...
    private Feather(Iterable<?> modules) {
        ...
        for (final Object module : modules) {
            if (module instanceof Class<?> klass) {
                throw new FeatherException(String.format("%s provided as class instead of an instance.", klass.getName()));
            }
    ...
    public void injectFields(Object target, MethodHandles.Lookup lookup) {
        ...
        for (FieldInjector f : injectFields.get(target.getClass())) {
            ...
            try {
                ...
                fieldSetter.invokeWithArguments(List.of(target, f.hasProvider ? provider(key, lookup) : instance(key, lookup)));
            } catch (Throwable t) {
    ...
    private <T> Provider<T> provider(final Key<T> key, Set<Key<?>> chain, MethodHandles.Lookup lookup) {
        if (!providers.containsKey(key)) {
            ...
            providers.put(key, singletonProvider(key, key.type.getAnnotation(Singleton.class), innerLookup -> {
                        try {
                            return innerLookup.unreflectConstructor(constructor)
                                    .invokeWithArguments(Arrays.asList(params(paramProviders, innerLookup)));
                        } catch (Throwable t) {
                            throw new FeatherException(String.format("Can't instantiate %s", key), t);
                        }
                    })
            );
        }
    ...
    private void providerMethod(final Object module, final Method m) {
        ...
        providers.put(key, singletonProvider(key, singleton, lookup -> {
                            try {
                                ...
                                throw new FeatherException(String.format("Can't instantiate %s with provider", key), t);
                            }
                        }
                )
        );
    ...
    private PermissionedProvider<?>[] paramProviders(
        ...
        for (int i = 0; i < parameterTypes.length; ++i) {
            ...
            if (providerType == null) {
                ...
                providers[i] = lookup -> provider(newKey, newChain, lookup).get();
            } else {
                final Key<?> newKey = Key.of(providerType, qualifier);
                providers[i] = lookup -> provider(newKey, lookup);
            }
        }
        return providers;
    }
